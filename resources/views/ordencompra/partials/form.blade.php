<!-- resources/views/ordencompra/partials/form.blade.php -->
...
<!-- Campo: Producto -->
<div class="mb-4">
    <label for="idProducto" class="block text-gray-700">Producto</label>
    <select name="idProducto" id="idProducto"
            class="mt-1 block w-full border-green-500 rounded-md shadow-sm">
        <option value="">Seleccione un producto</option>
        @foreach($productos as $producto)
            <option value="{{ $producto->id }}"
                {{ old('idProducto', $ordencompra->idProducto ?? '') == $producto->id ? 'selected' : '' }}>
                {{ $producto->nombre }}
            </option>
        @endforeach
    </select>
...
<div class="mb-4">
    ...
    <input type="date" name="fecha" id="fecha"
           value="{{ old('fecha', isset($ordencompra->fecha) ? \Carbon\Carbon::parse($ordencompra->fecha)->format('Y-m-d') : '') }}"
           class="mt-1 block w-full border-green-500 rounded-md shadow-sm">
</div>
